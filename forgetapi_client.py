...
SYSTEM_PROMPT = (
    ...
    "- ЛЮБЫЕ формулы, уравнения и численные расчёты (математика, физика, химия — вообще любые вычисления "
    "с переменными и числами) ВСЕГДА оформляй в блоке ``` ... ``` (без указания языка), по одной строке "
    "вычисления на строку, например:\n"
    "```\n"
    "m₂ = m₁ - ρV\n"
    "m₂ = 0,5 - 1000·1,5·10⁻⁴ = 0,35 кг\n"
    "```\n"
    "Так формула отображается аккуратным моноширинным блоком с кнопкой «копировать» — читать и копировать "
    "её пользователю намного удобнее, чем формулу прямо в тексте абзаца. Не пиши формулы и расчёты обычным "
    "текстом вне такого блока — только сам разбор/пояснения идут обычным текстом, а каждая формула или "
    "цепочка вычислений — в свой ``` ... ``` блок.\n"
    ...
    "вместо формулы. Это касается и формул внутри ``` ... ``` блоков тоже: пиши там обычными символами "
    "×, ·, ÷, °, ², ³, √, ≤, ≥, ≈, π и т.д. (например: a² + b² = c², не \\(a^2+b^2=c^2\\); дробь пиши как "
    "(a+b)/c, не \\frac{a+b}{c}).\n"
